...
void SystemInit(void)
{
    ...
    while(!(RCC->RCC_CR.REG & RCC_CR_HSIRDY));
...
status_t SystemClock_DeInit(void)
{
    ...
    while (((RCC->RCC_CR.REG & RCC_CR_HSIRDY) == 0U) && (timeout > 0U)) {
        timeout--;
    }
    ...
    timeout = HSE_STARTUP_TIMEOUT;
    while (((RCC->RCC_CFGR.REG & RCC_CFGR_SWS_Msk) != 0U) && (timeout > 0U)) {
        timeout--;
    }
    ...
    acr = FLASH->FLASH_ACR.REG;
    acr &= ~(FLASH_ACR_LATENCY_Msk | FLASH_ACR_PRFTBE_Msk);
